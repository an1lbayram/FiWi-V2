    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div className="glass-panel" style={{ padding: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '20px' }}>
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginTop: '6px' }}>
            <span style={{ fontSize: 'clamp(2.5rem, 6vw, 3.5rem)', fontWeight: '900', color: getScoreColor(), lineHeight: '1' }}>
              {score}
            </span>
            <span style={{ fontSize: '1.2rem', fontWeight: '700', color: 'var(--text-muted)' }}>/ 100</span>
            <span className="badge" style={{ backgroundColor: `${getScoreColor()}20`, color: getScoreColor(), border: `1px solid ${getScoreColor()}`, fontSize: '1rem', padding: '4px 14px' }}>
              Grade {grade}
            </span>
          </div>
        </div>
        ...
        <div style={{
          ...
          height: '76px',
          borderRadius: '50%',
          border: `5px solid ${getScoreColor()}`,
          boxShadow: `0 0 25px ${getScoreColor()}40`,
          display: 'flex',
          alignItems: 'center',
          ...
          <ShieldCheck size={38} color={getScoreColor()} />
        </div>
      </div>
      ...
      <div className="glass-panel" style={{ padding: '20px' }}>
        ...
        {issues.length === 0 ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', background: 'rgba(0, 230, 118, 0.1)', borderRadius: '12px', border: '1px solid rgba(0, 230, 118, 0.2)' }}>
            <CheckCircle color="var(--accent-green)" size={20} style={{ flexShrink: 0 }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {issues.map((iss, idx) => (
              <div key={idx} style={{ padding: '14px', background: 'rgba(255, 82, 82, 0.08)', borderRadius: '12px', border: '1px solid rgba(255, 82, 82, 0.2)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px', marginBottom: '6px' }}>
                  <span style={{ fontWeight: '700', color: 'var(--accent-red)', fontSize: '0.9rem', wordBreak: 'break-word' }}>
                    [{iss.severity}] {iss.issue}
                  </span>
                  <span className="badge badge-amber">{iss.profile}</span>
                </div>
                <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', lineHeight: '1.4' }}>{iss.detail}</p>
              </div>
            ))}
          </div>
        )}
      ...
      {recommendations.length > 0 && (
        <div className="glass-panel" style={{ padding: '20px' }}>
          ...
          <ul style={{ paddingLeft: '18px', display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '0.88rem', color: 'var(--text-main)' }}>
