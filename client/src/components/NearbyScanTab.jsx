...
import { Radio, Signal, CheckCircle2, ChevronDown, ChevronUp, Shield, Zap } from 'lucide-react';
...
export default function NearbyScanTab({ nearbyData, onRescan }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div className="glass-panel" style={{ padding: '24px', background: 'linear-gradient(135deg, rgba(0, 242, 254, 0.1), rgba(127, 83, 172, 0.1))' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }}>
          <div style={{ background: 'var(--bg-card-inner)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{t('rec24GHz')}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '6px' }}>
              <span className="badge badge-cyan" style={{ fontSize: '1.1rem', padding: '6px 14px' }}>
          ...
          <div style={{ background: 'var(--bg-card-inner)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{t('rec5GHz')}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '6px' }}>
              <span className="badge badge-cyan" style={{ fontSize: '1.1rem', padding: '6px 14px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {networks.map((net) => {
          ...
          return (
            <div key={net.ssid} className="glass-panel" style={{ padding: '18px 22px' }}>
              
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  <Signal color={signalNum > 60 ? 'var(--accent-green)' : signalNum > 30 ? 'var(--accent-amber)' : 'var(--accent-red)'} size={22} />
                  <div>
                    <h4 style={{ fontSize: '1.05rem', fontWeight: '700', color: 'var(--text-main)' }}>{net.ssid}</h4>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '2px' }}>
                      <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                        {net.authentication} ({net.encryption})
                      </span>
                      <span className="mono" style={{ fontSize: '0.78rem', color: 'var(--accent-cyan)' }}>
                        {net.bssids.length} Access Point(s)
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {isExpanded && (
                <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <span style={{ fontSize: '0.8rem', fontWeight: '700', color: 'var(--text-muted)' }}>{t('bssidDetails')}:</span>
                  {net.bssids.map((b, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: 'var(--bg-card-inner)', borderRadius: '8px', fontSize: '0.85rem', border: '1px solid var(--border-color)' }}>
